...
class subvector_cl{
public:
    subvector_cl(){ // инициализация пустого недовектора (top и capacity по нулям, а mas это NULL)
        mas = NULL;
        top = 0;
        capacity = 0;
    }
    ...
    void push_back(int d){ // добавление элемента в конец недовектора с выделением дополнительной памяти при необходимости
        if (top < capacity){
            mas[top] = d;
            top += 1;
        }
    ...
    ~subvector_cl(){	// очистить всю используемую память, инициализировать недовектор как пустой
        this->clear();
        delete[] mas;
    }
private:
